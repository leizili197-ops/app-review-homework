...
### v2.0（当前版本）- 2026年7月
- **核心改进**：支持实时抓取 App Store 评论，不再依赖缓存文件
- 用户输入任意 App 链接，系统实时抓取并分析
- 优化了分析速度，200条评论分析时间缩短至30-60秒
- 修复了1.0版本中换App无法抓取的Bug
...
> v2.0 起，应用**不再依赖缓存**：分析完全基于实时抓取，抓取失败时走「手动粘贴评论」而非回退缓存。

- 实时抓取成功后，会顺手把评论保存为 `reviews_cache.json`，作为**可选离线参考产物**（非必需）
- 应用**不会**自动读取缓存来兜底分析；断网或抓取为 0 条时，界面会提示手动粘贴评论
- 侧边栏可查看缓存状态或手动清除（仅管理那个离线文件，不影响实时抓取）
